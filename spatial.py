...
class VisionSystem:
    ...
    def __init__(self):
        ...
        self.detQ = self.device.getOutputQueue("detections", 15, False)
        if DISPLAY_FACE:
            self.faceQ = self.device.getOutputQueue("face", 15, False)
        
        self.recognizeFaceQueue = Queue(maxsize = 1)
        self.displayFaceQueue = Queue(maxsize = 20)
        ...
        while True:
            ...
            if inDet is not None:
                for det in inDet.detections:
                    ...
                    #print(f"{det.spatialCoordinates.x}, {det.spatialCoordinates.y}, {det.spatialCoordinates.z}")
                    result = {
                        'name': "",
                        'conf': "",
                        'coords': center,
